...
var checkHtmlURL = function(htmlString, checksfile) {
    var $ = cheerioHtmlString(htmlString);
    return cheerioCheck($, checksfile);
};

var getCheckURLfn = function(checksfile) {
...
if(require.main == module) {
	...
	.parse(process.argv);
    var checkJson;
    if (program.file) {                       // synchronous
	checkJson = checkHtmlFile(program.file, program.checks);
	var outJson = JSON.stringify(checkJson, null, 4);
	console.log(outJson);
    }
    if (program.url) {                        // asynchronous
	var response2console = getCheckURLfn(program.checks);
